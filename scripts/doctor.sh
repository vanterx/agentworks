...
for cli in claude codex hermes; do
...
if ! command -v claude >/dev/null 2>&1 && ! command -v codex >/dev/null 2>&1 && ! command -v hermes >/dev/null 2>&1; then
  warn "no agent CLI (claude/codex/hermes) on PATH — worker/review loops can't run on this machine"
fi
  ...
  if [ -z "$existing" ]; then
    ...
    for l in "${REQUIRED_LABELS[@]}"; do
      if ! grep -qxF "$l" <<<"$existing"; then
        fail "label missing: '$l' (create it per docs/GETTING_STARTED.md)"
        missing=1
      fi
    done
  ...
  if jq -e . "$AUTONOMY_FILE" >/dev/null 2>&1; then
    ...
    if [ "$at_agent" = "true" ]; then
      ...
      if [ "$visibility" = "public" ]; then
        warn "agent_triage is enabled on a PUBLIC repo — any issue author can reach your triage agent; see SECURITY.md#auto-triage"
      fi
    fi
...
if [ -f "$REPO_DIR/AGENTS.md" ] && grep -q 'TODO(adopter)' "$REPO_DIR/AGENTS.md"; then
  warn "AGENTS.md still contains the TODO(adopter) placeholder — agents are working without project-specific context"
else
  pass "AGENTS.md has no template placeholder markers"
fi
