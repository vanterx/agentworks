...
# Agent selection
# ---------------------------------------------------------------------------
AGENT="${AW_AGENT:-claude}"          # claude | codex | hermes
MODEL="${AW_MODEL:-}"
...
preflight() {
  ...
  if [ "${RUNS_AGENT:-0}" = "1" ] && ! command -v "$AGENT" >/dev/null 2>&1; then
    err "agent CLI '$AGENT' is not on PATH (set AW_AGENT to claude|codex|hermes)"
    exit 1
  fi
...
#   ./script.sh [claude|codex|hermes] [--model <name>|--model=<name>|-m <name>]
# CLI flags win over env vars and config files.
# ---------------------------------------------------------------------------
parse_agent_args() {
  while [ $# -gt 0 ]; do
    case "$1" in
      claude|codex|hermes) AGENT="$1" ;;
      --model) MODEL="${2:-}"; shift ;;
      --model=*) MODEL="${1#--model=}" ;;
      -m) MODEL="${2:-}"; shift ;;
      --dry-run) DRY_RUN=1 ;;
...
run_agent() {  # $1 = prompt, $2 = working dir (defaults to $REPO_DIR)
  ...
  case "$AGENT" in
    ...
    hermes)
      ...
      ;;
    *)
      err "unknown AGENT '$AGENT' (expected claude|codex|hermes)"
      return 2
      ;;
  esac
}
